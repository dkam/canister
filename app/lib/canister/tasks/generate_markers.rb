  ...
  def start
    ...
    @scene.scene_markers.each { |marker|
      ...
      if !File.exist?(marker_path)
        cmd = "ffmpeg -v quiet -ss #{marker.seconds.to_i} -t #{duration_mp4} -i \"#{@scene.path}\" -c:v libx264 -profile:v high -level 4.2 -preset veryslow -crf 24 -movflags +faststart -threads 4 -vf scale=#{output_width}:-2 -sws_flags lanczos -c:a aac -b:a 64k -strict -2 '#{tmp_marker_path}'"
      ...
      if !File.exist?(marker_path)
        cmd = "ffmpeg -v quiet -ss #{marker.seconds.to_i} -t #{duration_webp} -i \"#{@scene.path}\" -c:v libwebp -lossless 1 -q:v 70 -compression_level 6 -preset default -loop 0 -threads 4 -vf scale=#{output_width}:-2,fps=12 -an '#{tmp_marker_path}'"
