class Videos::StreamsController < ApplicationController
  ...
  def stream
    return head :not_found if @video.remote?
    Rails.logger.debug "Range header: #{request.headers["Range"]}"
    send_file @video.stream_file_path, disposition: "inline"
  end

  # Progressive MP4 streaming — FFmpeg pipe, fragmented MP4, smart codec selection
  def stream_mp4
    stream_config = @video.available_streams.find { |s| s[:kind] == :progressive && s[:mime_type] == "video/mp4" }

    return head :not_found unless stream_config

    start_time = params[:start].to_f

    Rails.logger.info "Streaming video #{@video.id} as MP4 (video_copy: #{stream_config[:video_copy]}, audio_transcode: #{stream_config[:audio_transcode]}, start: #{start_time})"
    ...
    Rails.logger.debug "Client disconnected during MP4 stream for video #{@video.id}"
  rescue => e
    Rails.logger.error "Error during MP4 stream for video #{@video.id}: #{e.class} - #{e.message}"
    raise e
  ensure
  ...
  def stream_hls
    stream_config = @video.available_streams.find { |s| s[:kind] == :hls }
    return head :not_found unless stream_config

    manifest = Hls::ManifestBuilder.resolve(@video, stream_config) do |segment_idx|
      stream_hls_segment_video_url(@video, segment: segment_idx)
    end
  ...
  def stream_hls_segment
    ...
    config = @video.available_streams.find { |s| s[:kind] == :hls }
    return head :not_found unless config

    result = Hls::StreamManager.instance.request_segment(@video, segment, config)
    if result == :ok
      send_file Hls::StreamManager.instance.cache.hls_dir(@video.id).join("#{segment}.ts"),
